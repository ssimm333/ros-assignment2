// Include our header file that declares the DriveX class
...
DriveX::DriveX(const std::string& name, const BT::NodeConfiguration& config)
    : BT::StatefulActionNode(name, config)  // Call base class constructor
{
    ...
    // Create a ROS2 action client for the "drive_x" action
    // This will communicate with an action server that controls the robot's movement
    // The action server must be running for this to work
    action_client_ = rclcpp_action::create_client<DriveXAction>(node_, "drive_x");
    
    // Initialize all state flags to their default values
    // These track the lifecycle of the action goal
    ...
    last_distance_ = 0.0;     // Track previous distance (for stuck detection)
    distance_diff_ = 0.0;     // Track change in distance (for progress monitoring)
}

// onStart() is called THE FIRST TIME this node is ticked by the behavior tree
// This is where we set up and send the drive action goal to the robot
BT::NodeStatus DriveX::onStart()
{
    // Reset success flag at the start of each execution
    success_ = false;
    
    // Check if the required "distance" input port has a value
    // has_value() returns true if the optional contains a value
    if(!getInput<double>("distance").has_value()) {
        // Log error - this input is mandatory, can't drive without knowing how far
        RCLCPP_ERROR(node_->get_logger(), "DriveX: Missing required input: distance");
        return BT::NodeStatus::FAILURE;
    }

    // Read input values from the ports (which read from the blackboard)
    distance_ = getInput<double>("distance").value();        // Required, already checked above
    speed_ = getInput<double>("speed").value_or(0.3);       // Optional, default 0.3 m/s
    tolerance_ = getInput<double>("tolerance").value_or(0.1); // Optional, default 0.1 m
    
    // Log the parameters we're using (helpful for debugging)
    RCLCPP_INFO(node_->get_logger(), "DriveX: Starting with distance: %f, speed: %f, tolerance: %f", distance_, speed_, tolerance_);
    ...
    if(!action_client_->wait_for_action_server(std::chrono::seconds(5))) {
        ...
        RCLCPP_ERROR(node_->get_logger(), "DriveX: Action server not available after waiting");
        return BT::NodeStatus::FAILURE;
    } 

    // Create the action goal message with our parameters
    DriveXAction::Goal goal;
    goal.distance = distance_;    // How far to drive
    goal.speed = speed_;          // How fast to drive
    goal.tolerance = tolerance_;  // How close is close enough
    
    // Create options struct to configure callbacks for the async action
    auto send_goal_options = rclcpp_action::Client<DriveXAction>::SendGoalOptions();
    ...
    send_goal_options.goal_response_callback = [this](GoalHandleDriveX::SharedPtr goal_handle) {
        // Clear the "goal_sent" flag since we now have a response
        goal_sent_ = false;
        
        // Check if goal was rejected (goal_handle will be null)
        if(!goal_handle) {
            RCLCPP_ERROR(rclcpp::get_logger("DriveX"), "DriveX: Goal was rejected by the action server");
            goal_active_ = false;
        } else {
            // Goal was accepted! Save the handle and mark as active
            RCLCPP_INFO(rclcpp::get_logger("DriveX"), "DriveX: Goal accepted by the action server, waiting for result");
            goal_active_ = true;
            goal_handle_ = goal_handle;  // Store handle so we can cancel later if needed
        }
    };
    
    // Set callback for when the action completes (success or failure)
    // This lambda is called when the action server finishes executing our goal
    send_goal_options.result_callback = [this](const GoalHandleDriveX::WrappedResult& result) {
        ...
        if(result.result && result.result->success) {
            RCLCPP_INFO(node_->get_logger(), "DriveX: Goal succeeded");
            success_ = true;   // Mark success so onRunning() returns SUCCESS
        } else {
            RCLCPP_ERROR(node_->get_logger(), "DriveX: Goal failed");
            success_ = false;  // Mark failure so onRunning() returns FAILURE
        }
    };

    // Set callback for periodic feedback while action is executing
    // This is called regularly by the action server to report progress
    // Currently empty, but could be used to monitor progress or detect stuck robot
    send_goal_options.feedback_callback = [this](GoalHandleDriveX::SharedPtr, const std::shared_ptr<const DriveXAction::Feedback> feedback) {
        // Could track feedback here, e.g., current_distance = feedback->remaining_distance
    };
...
// This checks the current state of the action and returns appropriate status
// The behavior tree will keep calling this until it returns SUCCESS or FAILURE
BT::NodeStatus DriveX::onRunning()
{
    ...
    if(goal_active_) {
        return BT::NodeStatus::RUNNING;  // Keep waiting for action to complete
    }
    
    // Phase 3: Goal has completed (result_callback was called)
    // Check the success_ flag that was set by the result_callback
    // Return SUCCESS if robot reached the target, FAILURE if it failed
...
void DriveX::onHalted()
{
    // Check if we have an active goal that needs to be cancelled
    if(goal_active_ && goal_handle_) {
        // Send cancel request to the action server to stop the robot
        action_client_->async_cancel_goal(goal_handle_);
        RCLCPP_INFO(node_->get_logger(), "DriveX: Goal cancelled");
    }
    
    // Reset all state flags to clean state
    goal_active_ = false;
    success_ = false;
}
